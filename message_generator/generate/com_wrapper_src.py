...
def generate_com_wrapper_src(directory, include_extension, src_extension):
    """
    ...
    tab = '   '
    #open file
    f = open(directory + '/com_wrapper' + src_extension, 'w')
    f.write('/* This file was autogenerated. All changes will be undone. */\n\n')
    f.write('#include <com_wrapper'+include_extension+'>\n')
    f.write('using namespace Comnet;\n\n')
    #constructor
    f.write('Node::Node(uint8_t node_id)\n{\n')
    f.write(tab+'node = new comnet::node(node_id);\n')
    f.write('}\n\nNode::~Node()\n{\n')
    f.write(tab+'delete(node);\n}\n\n')
    f.write('void Node::Start()\n{\n')
    f.write(tab+'node->start();\n}\n\n')
    #send_message methods for over ride with extra bool emergency argument
...
void Node::EstablishSerialEndpoint(int8_t link_id, uint8_t node_id)
{
    ...
    for message in protocol:
        ...
        variable_name = lower_case_acronym(name)
        type_t_name = variable_name + "_t"
        f.write('void* Node::'+cs_name+'Helper(int8_t link_id, com_header_t header, '+type_t_name+' '+variable_name+', comnet::node* ptr)\n{\n')
        ...
        f.write(tab+'node->register_on_'+variable_name+'(static_cast<comnet::'+variable_name+'_callback>(On'+cs_name+'Ptr.ToPointer()));\n')
        f.write('}\n\n')
