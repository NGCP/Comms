...
*C++ codem ust include cryptlib.lib
	...
	- Created an array pack and unpack for comnet marshall
	...
	- In comnet_comcol.h changed size of msg_data_offset which is the size of the header struct
	- In com+parser changed literal 12 to use msg_data_offset constant from comnet_comcol.h
	- size of msg_data_offset doesnt seem to matter, but it should
	- comnet will load file from key.txt to read in 16 byte encryption key
	- removed queue macros
	- com wrapper has been changed to handle c++ array in header struct
