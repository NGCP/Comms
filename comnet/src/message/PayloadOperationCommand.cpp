...
/* Message: Payload_Operation_Command, ID: 3002*/
...
msg_offset pack_payload_operation_command(
   ...
   msg_offset offset)
{
   offset = pack_float64_t(payload_operation_command->timestamp,offset);
   offset = pack_uint16_t(payload_operation_command->payload_ID,offset);
   offset = pack_uint8_t(payload_operation_command->payload_command,offset);
   return offset;
}
...

{
   ...
   msg->header.message_type = Com_Payload_Operation_Command;
   ...
   offset = pack_payload_operation_command(tx_msg, offset);
