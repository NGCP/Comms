...
/* Message: Communications_Payload_Command, ID: 3003*/

#include <comnet_marshal.h>
#include <comnet_message.h>
#include <message/CommunicationsPayloadCommand.h>

msg_offset pack_communications_payload_command(
   communications_payload_command_t* communications_payload_command,
   msg_offset offset)
{
   offset = pack_float64_t(communications_payload_command->timestamp,offset);
   offset = pack_uint16_t(communications_payload_command->comms_payload_ID,offset);
   offset = pack_uint8_t(communications_payload_command->comms_payload_command,offset);
   return offset;
}

msg_offset unpack_communications_payload_command(
   msg_offset offset,
   communications_payload_command_t* out_ptr)
{
   offset = unpack_float64_t(offset, &out_ptr->timestamp);
   offset = unpack_uint16_t(offset, &out_ptr->comms_payload_ID);
   offset = unpack_uint8_t(offset, &out_ptr->comms_payload_command);
   return offset;
}

void encode_communications_payload_command(
   uint8_t src_id,
   uint8_t dest_id,
   communications_payload_command_t* tx_msg,
   com_msg_t* msg,
...

{
   ...
   msg->header.message_type = Com_Communications_Payload_Command;
   ...
   offset = pack_communications_payload_command(tx_msg, offset);
