/* This file was autogenerated. All changes will be undone. */

/* Message: Air_Vehicle_Ground_Relative_State, ID: 2005*/

#include <comnet_marshal.h>
#include <comnet_message.h>
#include <message/AirVehicleGroundRelativeState.h>

msg_offset pack_air_vehicle_ground_relative_state(
   air_vehicle_ground_relative_state_t* air_vehicle_ground_relative_state,
   msg_offset offset)
{
   offset = pack_float64_t(air_vehicle_ground_relative_state->timestamp,offset);
   offset = pack_uint16_t(air_vehicle_ground_relative_state->vehicle_ID,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->angle_of_attack,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->angle_of_sideslip,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->true_airspeed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->indicated_airspeed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->north_wind_speed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->east_wind_speed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->north_ground_speed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->east_ground_speed,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->barometric_pressure,offset);
   offset = pack_float32_t(air_vehicle_ground_relative_state->barometric_altitude,offset);
   return offset;
}

msg_offset unpack_air_vehicle_ground_relative_state(
   msg_offset offset,
   air_vehicle_ground_relative_state_t* out_ptr)
{
   offset = unpack_float64_t(offset, &out_ptr->timestamp);
   offset = unpack_uint16_t(offset, &out_ptr->vehicle_ID);
   offset = unpack_float32_t(offset, &out_ptr->angle_of_attack);
   offset = unpack_float32_t(offset, &out_ptr->angle_of_sideslip);
   offset = unpack_float32_t(offset, &out_ptr->true_airspeed);
   offset = unpack_float32_t(offset, &out_ptr->indicated_airspeed);
   offset = unpack_float32_t(offset, &out_ptr->north_wind_speed);
   offset = unpack_float32_t(offset, &out_ptr->east_wind_speed);
   offset = unpack_float32_t(offset, &out_ptr->north_ground_speed);
   offset = unpack_float32_t(offset, &out_ptr->east_ground_speed);
   offset = unpack_float32_t(offset, &out_ptr->barometric_pressure);
   offset = unpack_float32_t(offset, &out_ptr->barometric_altitude);
   return offset;
}

void encode_air_vehicle_ground_relative_state(
   uint8_t src_id,
   uint8_t dest_id,
   air_vehicle_ground_relative_state_t* tx_msg,
   com_msg_t* msg,
   uint8_t keyArr[])//key for encryption CryptoPP

{
   /** Random IV*/
   Random::RandomNumberGen randIV = Random::RandomNumberGen();
   randIV.generate_numbers<uint8_t>(msg->header.iv,0, AES::BLOCKSIZE,0,255);

   msg_offset start_offset, offset;
   msg->direction = Com_Out;
   start_offset = msg->data;
   offset = msg->data;
   msg->header.node_src_id = src_id;
   msg->header.node_dest_id = dest_id;
   msg->header.message_length = 50;
   msg->header.message_type = Com_Air_Vehicle_Ground_Relative_State;
   offset = pack_sync(offset);
   offset = pack_header(&msg->header, offset);

   /** Encrypt */
   /* Managed C (CLR) will freak out if you pass SecByteBlock as an argument this is my half ass fix MW*/
   CryptoPP::SecByteBlock key(keyArr, AES::DEFAULT_KEYLENGTH);
   CFB_Mode<AES>::Encryption cfbEncryption(key, key.size(), msg->header.iv);
   cfbEncryption.ProcessData((byte*)offset, (byte*)offset, msg->header.message_length);

   offset = pack_air_vehicle_ground_relative_state(tx_msg, offset);
   offset = pack_checksum(start_offset, offset);
   msg->tx_len = offset-start_offset;
}