...
msg_offset pack_payload_data_recorder_command(
   ...
   msg_offset offset)
{
   offset = pack_float64_t(payload_data_recorder_command->timestamp,offset);
   offset = pack_uint16_t(payload_data_recorder_command->payload_ID,offset);
   offset = pack_uint8_t(payload_data_recorder_command->payload_recorder_mode,offset);
   return offset;
}
   ...
   payload_data_recorder_command_t* out_ptr)
{
   ...
   offset = unpack_uint8_t(offset, &out_ptr->payload_recorder_mode);
   return offset;
}
...

{
   ...
   msg->header.message_type = Com_Payload_Data_Recorder_Command;
   ...
   offset = pack_payload_data_recorder_command(tx_msg, offset);
