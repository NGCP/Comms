...
/* Message: UAV_Heading, ID: 6002*/

#include <comnet_marshal.h>
#include <comnet_message.h>
#include <message/UAVHeading.h>

msg_offset pack_UAV_heading(
   UAV_heading_t* UAV_heading,
   msg_offset offset)
{
   offset = pack_float64_t(UAV_heading->timestamp,offset);
   offset = pack_float64_t(UAV_heading->heading,offset);
   return offset;
}

msg_offset unpack_UAV_heading(
   msg_offset offset,
   UAV_heading_t* out_ptr)
{
   offset = unpack_float64_t(offset, &out_ptr->timestamp);
   offset = unpack_float64_t(offset, &out_ptr->heading);
   return offset;
}

void encode_UAV_heading(
   uint8_t src_id,
   uint8_t dest_id,
   UAV_heading_t* tx_msg,
   com_msg_t* msg,
...

{
   ...
   msg->header.message_type = Com_UAV_Heading;
   ...
   offset = pack_UAV_heading(tx_msg, offset);
